...
const mockStaffData: Staff[] = [
  ...
  },
  {
    ...
    rating: 4.7,
    experience: 5,
    skills: ["UI/UX Design", "Figma", "React", "CSS", "Adobe Creative Suite"],
    location: "Austin, TX",
    hourlyRate: 70,
    availability: "Busy",
...
export default function FindStaffPage() {
  ...
  useEffect(() => {
    ...
    if (filters.skills.length > 0) {
      filtered = filtered.filter((staff) => filters.skills.some((skill) => staff.skills.includes(skill)))
    }

    if (filters.location) {
      filtered = filtered.filter((staff) => staff.location.toLowerCase().includes(filters.location.toLowerCase()))
    }

    if (filters.minRating > 0) {
      filtered = filtered.filter((staff) => staff.rating >= filters.minRating)
    }

    if (filters.maxRate < 200) {
      filtered = filtered.filter((staff) => staff.hourlyRate <= filters.maxRate)
    }

    if (filters.minExperience > 0) {
      filtered = filtered.filter((staff) => staff.experience >= filters.minExperience)
    }

    if (filters.availability.length > 0) {
      filtered = filtered.filter((staff) => filters.availability.includes(staff.availability))
    }
        ...
        <div className="grid lg:grid-cols-4 gap-6">
          ...
          {showFilters && (
            <div className="lg:col-span-1">
              <StaffFilters filters={filters} onFilterChange={handleFilterChange} staffData={staffData} />
            </div>
          )}
